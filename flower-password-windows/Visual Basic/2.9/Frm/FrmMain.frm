...
Public Sub ShowFrmByHotKey()
    ...
    If FrmSetting.Visible Then
        MsgBox "请先关闭系统设置窗口, 再尝试按快捷键显示花密!", vbCritical + vbOKOnly + vbSystemModal
    Else
...
Private Sub Form_Initialize()
    ...
    If Not isRunInIDEMode Then
        If App.PrevInstance Then
            MsgBox "花密已经运行, 请尝试结束后台进程再重新打开!", vbCritical + vbOKOnly + vbSystemModal
            End
        Else
...
Private Sub Form_Load()
    ...
    If SetHotKey(Me.hwnd) = 0 Then
        MsgBox "花密注册系统全局快捷键 (WIN + S) 失败!", vbCritical + vbOKOnly + vbSystemModal
    ...
    If AddToTray(FrmMain, App.Title) = 0 Then
        ...
        End
    Else
        SetTrayMsgbox "按快捷键【WIN + S】显示花密, 在主面板按 Enter 键或 Esc 键隐藏花密.", NIIF_GUID, "欢迎使用花密 Windows 桌面版", Me.Icon
